...
sap.ui.define(
	[
		...
		"DecisionsOne/model/Models",
	],
	function (BaseController, MessageToast, RestModel, formatter, Models) {
	"use strict";

	return BaseController.extend("DecisionsOne.src.app.App", {
		fmt:formatter,
		_toolPage:{},
		onInit : function(){
		...
		loggedPopOver : function(oEvent){
			...
			if (!this._oPopoverLogged) {
			    this._oPopoverLogged = sap.ui.xmlfragment("DecisionsOne.src.pages.security.Logged", this);
		...
		createNavigation(){
			...
			model.get("webapp/model/data/AppModel.json")
			.then(
				(data)=>{
