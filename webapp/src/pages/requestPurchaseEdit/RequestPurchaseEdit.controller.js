sap.ui.define(
	...
	"use strict";
		...
		return BaseController.extend("DecisionsOne.src.pages.requestPurchaseEdit.RequestPurchaseEdit", {
			onInit : function () {							
				var oRouter;				                                  
				oRouter = this.getRouter();
				this._serviceUrl= this.getBusinessOneService();
				oRouter
				.getRoute('requestPurchaseEdit')
			...
			onSavePress(){
				...
				let path =this._serviceUrl +"/PurchaseRequests("+ model.getProperty("/DocEntry") +")"  
				model.patch(path)
				.then(this.dealUpdateResult().success)
				.catch(this.dealUpdateResult().error);
			...
			onAddItem: function(oEvent){			
				...
				dialog.addStyleClass(this.getOwnerComponent().getContentDensityClass());
				let url = this.getB1sUrl('Items')
				let model = new RestModel(); 
				
				let that = this;
				model.get(url,buttonFire).then(
						function(data) {
